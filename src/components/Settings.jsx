...
export default function Settings() {
    ...
    const handleClearCache = async () => {
        ...
        // 1. Clear localStorage app cache
        localStorage.removeItem('pingpong_events')

        // 2. Clear all Cache Storage entries (removes old Workbox caches)
        if ('caches' in window) {
            const keys = await caches.keys()
            await Promise.all(keys.map((k) => caches.delete(k)))
        }

        // 3. Unregister all service workers then re-register the notification one
